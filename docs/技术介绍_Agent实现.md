...
## 一句话（定位，先讲这个）
我们**以检索为核心**：用户输入先经三路引擎（规则 + 语义向量 + 行为红旗）**检索案例库、直接出风险卡片**，毫秒级、可解释、**绝大多数情况根本不需要大模型**。**只有少数难例**（信息不全、要多轮追问查证）才升级到**唯一的一个 LLM Agent**。

> 关键态度：**"能用检索解决就不调大模型"是刻意的设计**——省成本、低延迟、可控、可离线、隐私好。我们**只用了一个 Agent，而且让它少出场**，这是工程判断，不是功能少。

## 术语别用错（重要，免得被挑）
- **主路径不是 RAG，是"检索 + 规则/向量匹配"**：三路引擎出的卡片是命中案例的结构化字段拼出来的，**不经大模型生成**，严格说叫"检索式研判"更准，别张口就说"全程 RAG"。
- **真正用到 LLM 的只有两类**：① H5「AI 深度研判」= 标准 **RAG**（把检索结果喂给大模型生成解释）；② `guardian_agent.py` = **Agentic RAG**（大模型自己调检索工具）。
- **真正的"Agent"只有一个**：`guardian_agent.py`。其余是"LLM 增强"，别都叫 Agent。
...
**Q：你们是"多 Agent / Agent 系统"吗？用了几个 Agent？**
不是，也不想夸大。**我们只用了一个 Agent**，而且**绝大多数情况根本不调它**——主路径是三路检索直接出卡片就够了。Agent 只在难例（信息不全、要追问查证）时才升级。**"能检索解决就不上大模型"是我们刻意的取舍**：省成本、低延迟、可控、可离线、隐私好。Agent 不是越多越好，按需用才是工程判断。

**Q：那核心到底是检索还是大模型？**
**核心是检索**（三路引擎 + 案例库），大模型是**难例时的增强**。所以系统离线也能跑、可解释、便宜；大模型只在少数情况锦上添花。
